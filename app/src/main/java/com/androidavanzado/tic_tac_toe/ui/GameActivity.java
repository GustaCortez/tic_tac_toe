...
import com.androidavanzado.tic_tac_toe.app.Constantes;
import com.androidavanzado.tic_tac_toe.model.Jugada;
...
import com.androidavanzado.tic_tac_toe.databinding.ActivityGameBinding;
...
public class GameActivity extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        casilllas = new ArrayList<>();
        casilllas.add((ImageView) findViewById(R.id.imageView0));
        casilllas.add((ImageView) findViewById(R.id.imageView1));
        casilllas.add((ImageView) findViewById(R.id.imageView2));
        casilllas.add((ImageView) findViewById(R.id.imageView3));
        casilllas.add((ImageView) findViewById(R.id.imageView4));
        casilllas.add((ImageView) findViewById(R.id.imageView5));
        casilllas.add((ImageView) findViewById(R.id.imageView6));
        casilllas.add((ImageView) findViewById(R.id.imageView7));
        casilllas.add((ImageView) findViewById(R.id.imageView8));
        ...
        listenerJugada = db.collection("jugadas")
                ...
                .addSnapshotListener(GameActivity.this, new EventListener<DocumentSnapshot>() {
                    ...
                    if (snapshot.exists() && source.equals("Server")){
                        //parseando de documentsnapshot a jugada
                        jugada = snapshot.toObject(Jugada.class);
                        if (playerOneName.isEmpty() || playerTwoName.isEmpty()){
                            // obtener los nombres de usuarios de la jugada
                            getPlayerNames();
    ...
    private void updatePlayersUI() {
        if (jugada.isTurnojugadorUno()){
            tvPlayer1.setTextColor(getResources().getColor(com.google.android.material.R.color.primary_material_dark));
            tvPlayer2.setTextColor(getResources().getColor(com.google.android.material.R.color.design_default_color_primary));
        }else{
            tvPlayer1.setTextColor(getResources().getColor(com.google.android.material.R.color.design_default_color_primary));
            tvPlayer2.setTextColor(getResources().getColor(com.google.android.material.R.color.primary_dark_material_light));
    ...
    private void updateUI() {
        for (int i =0; i <9; i++){
            ...
            ImageView ivCasillaActual= casilllas.get(i);
            if (casilla ==0){
                ivCasillaActual.setImageResource(R.drawable.square);
            }else if(casilla==1){
                ivCasillaActual.setImageResource(R.drawable.player_one);
            }else{
                ivCasillaActual.setImageResource(R.drawable.player_two);
            }
        }
  ...
  public  void casillaSeleccionada(View view) {
    ...
    private void actualizarJugada(String numeroCasilla) {
        ...
        if (jugada.getCeldasSeleccionadas().get(posicionCasilla)!=0){
            ...
            if (jugada.isTurnojugadorUno()) {
                casilllas.get(posicionCasilla).setImageResource(R.drawable.player_one);
                jugada.getCeldasSeleccionadas().set(posicionCasilla, 1);
            } else {
                casilllas.get(posicionCasilla).setImageResource(R.drawable.player_two);
            ...
            //Actualizar en firestore los datos dela jugada
            db.collection("jugadas")
                    ...
                    .addOnSuccessListener(GameActivity.this, new OnSuccessListener<Void>() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Log.w("ERROR", "Error al gurdar la jugadad");
...
//obtenermos la referencias al construit layout
        ...
        if (ganadorId.equals("EMPATE")){
            ...
            gameOverAnimation.setAnimation("game_over.json");
        }
